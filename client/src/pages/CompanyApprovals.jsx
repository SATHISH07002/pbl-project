...
export default function CompanyApprovals() {
  ...
  const fetchList = () => {
    API.get("/internships/company/pending")
  ...
  const handleApprove = async (id) => {
    setActionLoading(id);
    try {
      await API.post(`/internships/company/${id}/approve`);
      fetchList();
    } catch (err) {
  ...
  const handleReject = async (id) => {
    ...
    setActionLoading(id);
    try {
      await API.post(`/internships/company/${id}/reject`, { reason });
    ...
    <Layout title="Pending Approvals">
      <div className="card">
        ...
        <p style={{ color: "var(--color-text-muted)", marginBottom: "1rem" }}>
          View internship details and college approval status. On approve: Certificate ID + QR Code generated.
        </p>
        {loading ? (
          <p>Loading...</p>
        ) : list.length === 0 ? (
            ...
            {list.map((item) => (
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem" }}>
                  <div>
                    ...
                    <p style={{ fontSize: "0.9rem", color: "var(--color-text-muted)" }}>
                      College: {item.college?.collegeName || item.college?.name} (Approved)
                    </p>
                    <p>Role: {item.roleInInternship} | Duration: {item.duration} days</p>
                  </div>
                  <div>
                ...
                <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.5rem" }}>
                  ...
                  >
                    {actionLoading === item._id ? "..." : "Approve & Generate Certificate ID"}
                  </button>
                  <button
